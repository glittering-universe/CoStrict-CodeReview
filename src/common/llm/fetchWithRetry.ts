...
export const createRetryingFetcher = ({
  maxRetries = getEnvNumber('COSTRICT_FETCH_MAX_RETRIES') ?? 5,
  baseDelayMs = getEnvNumber('COSTRICT_FETCH_BASE_DELAY_MS') ?? 1000,
      ...
      try {
        ...
        if (!response.ok && shouldRetryStatus(response.status) && attempt < maxRetries) {
          if (response.status === 429) {
            try {
              ...
              if (isNonRetryable429Response(bodyText)) {
                ...
                headers.set('x-costrict-original-status', '429')
          ...
          const waitMs = Math.min(
            ...
            getEnvNumber('COSTRICT_FETCH_MAX_BACKOFF_MS') ?? 120_000
          )
