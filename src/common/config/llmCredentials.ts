...
const getDefaultCredentialPaths = async (): Promise<string[]> => {
  ...
  if (process.env.COSTRICT_LLM_CREDENTIALS_PATH) {
    paths.push(process.env.COSTRICT_LLM_CREDENTIALS_PATH)
  }

  paths.push(join(homedir(), '.costrict', 'credentials.json'))

  try {
    const gitRoot = await getGitRoot()
    paths.push(join(gitRoot, '.costrict', 'credentials.json'))
  } catch {
...
export const writeProjectCredentials = async (
  ...
  const gitRoot = await getGitRoot()
  const configPath = join(gitRoot, '.costrict', 'credentials.json')
  const payload = {
