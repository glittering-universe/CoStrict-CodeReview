...
const COSTRICT_DIR_NAME = '.costrict'
const COSTRICT_CONFIG_DIR = join(homedir(), COSTRICT_DIR_NAME)
const REPO_IDS_FILE = join(COSTRICT_CONFIG_DIR, 'repo_ids.json')
...
export const localProvider = async (): Promise<PlatformProvider> => {
  ...
  const costrictDirPath = join(workspaceRoot, COSTRICT_DIR_NAME)
  const reviewDirPath = join(costrictDirPath, 'review')
  const reviewFilePath = join(reviewDirPath, reviewFileName)
  const gitignorePath = join(reviewDirPath, '.gitignore')

  const ensureCostrictDirExists = async (): Promise<void> => {
    try {
  ...
  const updateRepoIdMapping = async (
    ...
    try {
      await mkdir(COSTRICT_CONFIG_DIR, { recursive: true })
